...
public abstract class RobotTestBed extends BattleAdaptor {
    ...
    public void run() {
        ...
        runTeardown();
        MatcherAssert.assertThat(errors, is(getExpectedErrors()));
    }
    ...
    private void runBattle(String robotList, int numRounds, String initialPositions) {
        ...
        if (getExpectedRobotCount(robotList) > 0) {
            Assert.assertNotNull("Robot not loaded", robotSpecifications);
            Assert.assertEquals("Robot not loaded", getExpectedRobotCount(robotList),
